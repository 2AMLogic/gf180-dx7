...
3. **Round-trip pin**: `tools/verify_gf180_7t_techmaps.py` re-derives each
   ...
   real-PDK check when the pinned PDK is mounted (`~/.ciel` is a default
   search location as of the H10 increment).

4. **License basis (recorded gap).** The pinned image states the ORFS
   repository license for its build/run scripts as BSD 3-Clause
   (`/OpenROAD-flow-scripts/LICENSE_BUILD_RUN_SCRIPTS`, `sha256
   fae0db7a4c00c3125f037e5818e8cd0c8aa5c67fff2e8c558312068e9f2d1592`); its
   README says platform licenses live under `flow/platforms/<platform>/`,
   but the image's `flow/platforms/gf180/` contains **no** license file.
   The techmaps therefore ship under the nearest stated license: the full
   BSD 3-Clause text is committed alongside them
   (`asic/orfs/gf180_7t/LICENSE-ORFS-BSD-3-Clause`, byte-identical to the
   image file) and each techmap header carries the copyright line. The
   absence of a platform-level license file is a recorded gap for owner
   review, not a resolved attribution.

Re-verification re-run 2026-09-24 (H10 increment, issue #32): the image
was pulled by digest on the H10 host, both platform files re-extracted
(sha256 values above reproduced exactly), `tools/verify_gf180_7t_techmaps.py
--image-dir` = PASS (transform + lib check), against the ciel tt liberty
`sha256 330aa9439255...` (identical bytes to the H07 pin; ciel version
`f6eeac7dad085ffcc829ccfd721f7b4ce39edcf7` on this host).
