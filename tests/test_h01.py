...
Covers, per the issue's acceptance:
  ...
  - tools/h01_synth.py parse_stat asserts the single-module transcript
    shape its area number depends on (issue #95): the committed
    yosys_full.log replay still yields 46,939.9616 um^2, and a synthetic
...
def _h01_synth():
    """Import tools/h01_synth.py for the stat-parser unit tests."""
...
# per-module block (8,912.34 um^2 here) as the design area -- the #82/#94
# bug. Not committed evidence and not a measurement: a hand-built control.
MULTI_MODULE_TRANSCRIPT = """
=== operator_probe.op_kernel ===

      512 8.91E+03 cells
       33 2.10E+03   gf180mcu_fd_sc_mcu7t5v0__dffq_1

   Chip area for module '\\operator_probe.op_kernel': 8912.340000
     of which used for sequential elements: 2100.806400 (23.57%)

=== operator_probe ===

     2407 4.69E+04 cells
       99 6.30E+03   gf180mcu_fd_sc_mcu7t5v0__dffq_1

   Chip area for module '\\operator_probe': 46939.961600
     of which used for sequential elements: 6302.419200 (13.43%)
...
=== design hierarchy ===
        ...
        |        |
     2919 5.59E+04 operator_probe
      512 8.91E+03   operator_probe.op_kernel

     2919 5.59E+04 cells
      132 8.40E+03   gf180mcu_fd_sc_mcu7t5v0__dffq_1

   Chip area for top module '\\operator_probe': 55852.301600
     of which used for sequential elements: 8403.225600 (15.04%)
"""
...
class TestSynthStatSingleModuleGuard(unittest.TestCase):
    """tools/h01_synth.py parse_stat must ASSERT the single-module shape its
    area number silently depended on (issue #95, part of #91).

    operator_probe is a flat single-module design today: the committed
    evidence/h01-operator-probe/yosys_full.log carries exactly ONE
    ...
    46,939.9616 um^2 is correct. Nothing in the pre-#95 parser checked
    ...
    COMMITTED_AREA_UM2 = 46939.9616
    COMMITTED_SEQ_AREA_UM2 = 6302.4192
    COMMITTED_CELL_TOTAL = 2407
    COMMITTED_FLOP_TOTAL = 99
    # the per-module block a guard-less parser would hand back from the
    # synthetic multi-module transcript below
    SUBMODULE_LOCAL_AREA = 8912.34
    ...
    def test_synthparse_committed_log_single_module_keeps_area(self):
        ...
        self.assertEqual(log.count("Chip area for module"), 1,
                         "evidence/h01-operator-probe/yosys_full.log is no "
    ...
    def test_synthparse_multi_module_transcript_fails_loudly(self):
        ...
        the pre-#95 parser returned here (8,912.34 um^2)."""
        h = _h01_synth()
    ...
    def test_synthparse_strip_control_transcript_stays_parseable(self):
        ...
        stay parseable (zeros / None) or the negative control the H01 gate
        depends on could not be evaluated."""
        st = _h01_synth().parse_stat(self._log(self.STRIP_LOG))
