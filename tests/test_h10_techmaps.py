...
class TestTechmapVerifier(unittest.TestCase):
    ...
    def test_real_pdk_lib_check(self):
        default = (os.environ.get("GF180MCU_7T_TT_LIB", ""),
                   "/Users/joseph/dev/2amlogic/prep/tinytapeout/pdk/ciel/"
                   "gf180mcu/versions/54435919abffb937387ec956209f9cf5fd2"
                   "dfbee/gf180mcuD/libs.ref/gf180mcu_fd_sc_mcu7t5v0/lib/"
        ...
        if libp is None:
            self.skipTest("no local 7t tt liberty on this machine "
                          ...
                          "liberty)")
        tm = os.path.join(REPO_ROOT, "pnr", "orfs", "gf180_7t")
        r = self._run("--techmap-dir", tm, "--lib", libp)
        self.assertEqual(r.returncode, 0, r.stdout + r.stderr)
