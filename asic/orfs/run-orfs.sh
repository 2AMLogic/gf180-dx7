...
# H10: rtl/*.v $readmemh defaults resolve reference/tables/*.hex relative to the flow
# cwd inside the container; publish them read-only there (the RTL keeps absolute-path
# `ifdef overrides for other toolchains).
TAB="$REPO/reference/tables"
[ -d "$TAB" ] && MOUNTS+=(-v "$TAB:/OpenROAD-flow-scripts/flow/reference/tables:ro")
